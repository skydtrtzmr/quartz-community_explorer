...
/**
 * contentIndex.json 单个条目的运行时形状（自 v4 ContentDetails 简化而来）。
 * frontmatter 字段由本地插件 content-index-pro 写入，排序/过滤依赖它。
 */
...
// 自 v4 client/quartz/util/fileTrie.ts 整体移植（仅调整 import 来源）
export class FileTrieNode<T extends FileTrieData = ContentIndexEntry> {
  isFolder: boolean
  ...
  get slug(): FullSlug {
    const path = joinSegments(...this.slugSegments) as unknown as FullSlug
    if (this.isFolder) {
      return joinSegments(path, "index") as unknown as FullSlug
    }
